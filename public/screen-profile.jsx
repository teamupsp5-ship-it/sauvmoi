...
// ── Composant : sélecteur de langue (deux options, indication de la langue
// active) — au clic, bascule immédiatement toute l'app via window.SM_I18N.
function LangSwitch({ lang }) {
  // div[role=button], pas <button> : ce composant est utilisé comme `right`
  // d'un ListRow qui est LUI-MÊME un <button> — deux boutons imbriqués
  // seraient invalides en HTML (et le clic sur l'un déclencherait aussi
  // celui de l'autre).
  const pill = (code, native, active) => (
    <div
      key={code}
      role="button"
      tabIndex={0}
      onClick={(e) => { e.stopPropagation(); window.SM_I18N.set(code); }}
      style={{
        padding: '5px 12px', borderRadius: 999,
        cursor: 'pointer',
        fontSize: 12, fontWeight: 700, fontFamily: 'var(--font-ui)',
        background: active ? 'var(--sm-blue)' : 'var(--sm-paper-2)',
        color: active ? 'white' : 'var(--sm-ink-500)',
  ...
  return (
    <div style={{ display: 'flex', gap: 6 }}>
      {pill('fr', t('common.lang_fr_native'), lang === 'fr')}
      {pill('en', t('common.lang_en_native'), lang === 'en')}
    </div>
  );
}
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
      ...
      <div style={{ background: 'white', borderRadius: '20px 20px 0 0', maxHeight: '78%', display: 'flex', flexDirection: 'column', boxShadow: '0 -4px 30px rgba(0,0,0,0.12)' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px 32px' }}>
          {notifs.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '36px 20px', color: 'var(--sm-ink-500)', fontSize: 14, fontFamily: 'var(--font-ui)' }}>{t('home.no_notifications')}</div>
          ) : notifs.map(n => (
...
function ProfileScreen({ nav }) {
  ...
  async function changePassword() {
    if (!pwForm.next || pwForm.next.length < 6) return setPwMsg(t('profile.password_min_error'));
    if (pwForm.next !== pwForm.confirm)        return setPwMsg(t('profile.password_mismatch'));
    ...
    try {
      ...
      setPwMsg(res.ok ? t('profile.password_changed') : (data.error || t('common.error')));
      if (res.ok) { setPwForm({ old: '', next: '', confirm: '' }); setTimeout(() => setShowPw(false), 1200); }
    } catch { setPwMsg(t('common.network_error')); }
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 32px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ background: 'white', borderRadius: 'var(--sm-radius)', boxShadow: 'var(--sm-shadow)', padding: '18px 18px 20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 17, fontWeight: 700, color: 'var(--sm-ink)', fontFamily: 'var(--font-ui)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {user.name || t('profile.default_user_name')}
              </div>
          ...
          <div style={{ marginTop: 18 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: completion < 100 ? 'space-between' : 'flex-end', marginBottom: completion < 100 ? 8 : 0 }}>
              {completion < 100 ? (
                <>
                  <span style={{ fontSize: 13, color: 'var(--sm-ink-500)', fontFamily: 'var(--font-ui)' }}>{t('profile.completion_label')}</span>
                ...
                <div style={{ background: '#EAF3DE', borderRadius: 999, padding: '4px 12px', display: 'flex', alignItems: 'center', gap: 5 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 700, color: '#27500A', fontFamily: 'var(--font-ui)' }}>{t('profile.profile_complete')}</span>
                </div>
              )}
            </div>
            {completion < 100 && (
        ...
        <div style={{ background: 'white', borderRadius: 'var(--sm-radius)', boxShadow: 'var(--sm-shadow)', overflow: 'hidden' }}>
          ...
          <ListRow
            ...
            label={t('profile.medical_profile')}
            badge={medIncomplete}
            right={medIncomplete ? <span style={{ fontSize: 12, fontWeight: 600, color: '#D85A30', fontFamily: 'var(--font-ui)' }}>{t('profile.to_complete')}</span> : null}
          ...
          <ListRow
            ...
            label={t('profile.qr_medical_code')}
            right={
          ...
          <ListRow
            ...
            label={t('terms.header_title')}
            onClick={() => nav.go('terms')}
          />
          <ListRow
            icon="globe" iconBg="#F1F2F4" iconColor="var(--sm-ink)"
            label={t('profile.language')}
            right={<LangSwitch lang={lang} />}
            hideChevron
          />
          <ListRow
        ...
        <div style={{ background: 'white', borderRadius: 'var(--sm-radius)', boxShadow: 'var(--sm-shadow)', overflow: 'hidden' }}>
          <button onClick={() => setShowLogout(true)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
            ...
            <span style={{ flex: 1, fontSize: 15, fontWeight: 700, color: '#C0392B', fontFamily: 'var(--font-ui)' }}>{t('profile.logout')}</span>
          </button>
        </div>
      </div>
      ...
      {showPw && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 150, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
          ...
          <div style={{ background: 'white', borderRadius: '20px 20px 0 0', padding: '20px 20px 36px', boxShadow: '0 -4px 30px rgba(0,0,0,0.12)' }}>
            ...
            <h3 className="sm-serif" style={{ fontSize: 19, marginBottom: 16 }}>{t('profile.change_password')}</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {[{ label: t('profile.current_password'), key: 'old', ph: '••••••••' }, { label: t('profile.new_password'), key: 'next', ph: t('profile.new_password_hint') }, { label: t('profile.confirm_password'), key: 'confirm', ph: '••••••••' }].map(f => (
                <div key={f.key}>
      ...
      {showLogout && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,22,40,0.5)', display: 'flex', alignItems: 'flex-end', zIndex: 200 }}>
          <div style={{ width: '100%', background: 'white', borderRadius: '20px 20px 0 0', padding: '28px 20px 40px' }}>
            ...
            <h3 className="sm-serif" style={{ fontSize: 20, textAlign: 'center', marginBottom: 8 }}>{t('profile.logout_confirm_title')}</h3>
...
function ProfileMedical({ nav }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'var(--sm-paper)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <SubHeader
        title={t('profile.medical_profile')}
        nav={nav}
        right={!editing && (
          <button onClick={startEdit} style={{ padding: '7px 16px', borderRadius: 8, background: 'var(--sm-blue)', color: 'white', border: 'none', fontSize: 14, fontWeight: 600, fontFamily: 'var(--font-ui)', cursor: 'pointer' }}>
            {t('common.edit')}
        ...
        <div style={{ background: 'white', borderRadius: 'var(--sm-radius)', boxShadow: 'var(--sm-shadow)', overflow: 'hidden' }}>
          ...
          {editing && (
            <>
              ...
              <div style={{ padding: '12px 16px', background: '#F4F8FF' }}>
                <div style={{ fontSize: 12, color: 'var(--sm-blue)', fontWeight: 600, fontFamily: 'var(--font-ui)', marginBottom: 6 }}>{t('profile.conditions_hint')}</div>
...
function ProfileContacts({ nav }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'var(--sm-paper)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <SubHeader
        title={t('profile.emergency_contacts')}
        nav={nav}
        right={!editing && (
          <button onClick={startEdit} style={{ padding: '7px 16px', borderRadius: 8, background: 'var(--sm-blue)', color: 'white', border: 'none', fontSize: 14, fontWeight: 600, fontFamily: 'var(--font-ui)', cursor: 'pointer' }}>
            {t('common.edit')}
          ...
          <>
            {contacts.map((c, i) => (
              <div key={i} style={{ background: 'white', borderRadius: 'var(--sm-radius)', boxShadow: 'var(--sm-shadow)', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--sm-blue)', fontFamily: 'var(--font-ui)' }}>{t('profile.contact_n').replace('{n}', i + 1)}</span>
                ...
                <input value={c.name} onChange={e => updateContact(i, 'name', e.target.value)} placeholder={t('profile.contact_name_placeholder')} style={{ ...PINP }} />
                <input value={c.phone} onChange={e => updateContact(i, 'phone', e.target.value)} placeholder="+225 07 00 00 00" type="tel" style={{ ...PINP }} />
                <input value={c.relation || ''} onChange={e => updateContact(i, 'relation', e.target.value)} placeholder={t('profile.relation_placeholder')} style={{ ...PINP }} />
              </div>
            ))}
            ...
            {contacts.length < 5 && (
              <button onClick={addContact} style={{ background: 'white', border: '2px dashed var(--sm-line)', borderRadius: 'var(--sm-radius)', padding: '14px', color: 'var(--sm-blue)', fontWeight: 600, fontSize: 14, fontFamily: 'var(--font-ui)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                ...
                {t('profile.add_contact').replace('{n}', contacts.length)}
              </button>
            )}
          </>
        )}
