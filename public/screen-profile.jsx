...
const PINP = {
  ...
  fontSize: 15, fontFamily: 'inherit', color: 'var(--sm-ink)', width: '100%',
};

function PField({ label, value, onChange, type = 'text', placeholder, last }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px', borderBottom: last ? 'none' : '1px solid var(--sm-line)' }}>
      <span style={{ fontSize: 13, color: 'var(--sm-ink-500)', width: 100, flexShrink: 0 }}>{label}</span>
      <input
        type={type}
        value={value}
...
function ProfileScreen({ nav }) {
  ...
  // Couleur avatar déterministe selon initiales
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'var(--sm-paper)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px 10px', background: 'white', borderBottom: '1px solid var(--sm-line)', flexShrink: 0 }}>
        ...
        <span style={{ flex: 1, fontWeight: 700, fontSize: 16 }}>Mon profil</span>
        {!editing ? (
          <button onClick={startEdit} style={{ padding: '6px 14px', borderRadius: 8, background: 'var(--sm-soft-blue)', border: 'none', color: 'var(--sm-blue)', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>
            Modifier
          </button>
        ) : (
          <button onClick={cancelEdit} style={{ padding: '6px 14px', borderRadius: 8, background: 'var(--sm-paper-2)', border: 'none', color: 'var(--sm-ink-500)', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>
            Annuler
          </button>
        )}
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 40 }}>
        ...
        <PSection title="Informations personnelles">
          {!editing ? (
            ...
            <>
              ...
              <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--sm-line)' }}>
                <div style={{ fontSize: 13, color: 'var(--sm-ink-500)', marginBottom: 8 }}>Sexe</div>
                <div style={{ display: 'flex', gap: 8 }}>
                  {GENDERS_P.map(g => (
                    <button key={g} type="button" onClick={() => setF('gender')(g)} style={{ flex: 1, padding: '8px 4px', borderRadius: 8, fontSize: 13, fontFamily: 'inherit', cursor: 'pointer', background: form.gender === g ? 'var(--sm-ink)' : 'white', color: form.gender === g ? 'white' : 'var(--sm-ink)', border: `1.5px solid ${form.gender === g ? 'var(--sm-ink)' : 'var(--sm-line)'}` }}>
                      {g}
                    </button>
                  ))}
                </div>
        ...
        <PSection title="Profil médical">
          {!editing ? (
            ...
            <>
              <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--sm-line)' }}>
                <div style={{ fontSize: 13, color: 'var(--sm-ink-500)', marginBottom: 8 }}>Groupe sanguin</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: '1px solid var(--sm-line)' }}>
                <div style={{ padding: '10px 16px', borderRight: '1px solid var(--sm-line)' }}>
                  <div style={{ fontSize: 12, color: 'var(--sm-ink-500)', marginBottom: 4 }}>Taille (cm)</div>
                  <input type="number" value={form.height} onChange={e => setF('height')(e.target.value)} placeholder="170" min="50" max="250" style={{ ...PINP }} />
                </div>
                <div style={{ padding: '10px 16px' }}>
                  <div style={{ fontSize: 12, color: 'var(--sm-ink-500)', marginBottom: 4 }}>Poids (kg)</div>
                  <input type="number" value={form.weight} onChange={e => setF('weight')(e.target.value)} placeholder="65" min="1" max="300" style={{ ...PINP }} />
                </div>
              </div>
              <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--sm-line)' }}>
                <div style={{ fontSize: 12, color: 'var(--sm-ink-500)', marginBottom: 4 }}>Allergies (séparées par virgule)</div>
                <input value={form.allergies} onChange={e => setF('allergies')(e.target.value)} placeholder="Pénicilline, Arachides…" style={{ ...PINP }} />
              </div>
              <div style={{ padding: '10px 16px' }}>
                <div style={{ fontSize: 12, color: 'var(--sm-ink-500)', marginBottom: 4 }}>Antécédents médicaux</div>
                <textarea value={form.conditions} onChange={e => setF('conditions')(e.target.value)} placeholder="Diabète, hypertension…" rows={2} style={{ ...PINP, resize: 'none', lineHeight: 1.5 }} />
              </div>
            </>
          )}
        </PSection>
          ...
          {!editing ? (
            ...
            <>
              {form.contacts.map((c, i) => (
                <div key={i} style={{ padding: '10px 16px', borderBottom: '1px solid var(--sm-line)', display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: 12, color: 'var(--sm-ink-500)', fontWeight: 600 }}>Contact {i + 1}</span>
                  ...
                  <input value={c.name} onChange={e => updateContact(i, 'name', e.target.value)} placeholder="Nom du contact" style={{ ...PINP, border: '1px solid var(--sm-line)', borderRadius: 8, padding: '8px 10px' }} />
                  <input value={c.phone} onChange={e => updateContact(i, 'phone', e.target.value)} placeholder="+225 07 00 00 00" type="tel" style={{ ...PINP, border: '1px solid var(--sm-line)', borderRadius: 8, padding: '8px 10px' }} />
                  <input value={c.relation || ''} onChange={e => updateContact(i, 'relation', e.target.value)} placeholder="Relation (époux, sœur…)" style={{ ...PINP, border: '1px solid var(--sm-line)', borderRadius: 8, padding: '8px 10px' }} />
                </div>
              ))}
        ...
        {editing && (
          <div style={{ padding: '0 14px 18px' }}>
            <button onClick={saveProfile} disabled={saving} style={{ width: '100%', padding: '14px', borderRadius: 14, background: saving ? 'var(--sm-line)' : 'var(--sm-blue)', color: 'white', border: 'none', fontWeight: 700, fontSize: 16, cursor: saving ? 'default' : 'pointer' }}>
            ...
            <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--sm-line)', display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[
                ...
                <div key={f.key} style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  ...
                  <input type="password" value={pwForm[f.key]} onChange={e => setPwForm(p => ({ ...p, [f.key]: e.target.value }))} placeholder={f.placeholder} style={{ ...PINP, border: '1.5px solid var(--sm-line)', borderRadius: 8, padding: '9px 12px' }} />
                </div>
              ))}
