...
| `screen-map.jsx` | `MapScreen` — module Localisation : carte Leaflet + liste des centres de santé, chargée via `window.API.healthCenters(lat, lng)` (`GET /api/health-centers`, couverture nationale, voir section Localisation), position GPS temps réel via `navigator.geolocation.watchPosition`, distance Haversine (calculée côté client, redondante avec celle déjà renvoyée par le backend — comportement préexistant, inchangé), tri par proximité, filtres (Tous / Hôpitaux / Cliniques / Pharmacies / Centres de santé — chip actif en `var(--sm-navy-deep)`, cohérent avec la tabbar) **alignés sur la taxonomie à 5 catégories de `resolveType()`** (`scripts/sync-health-centers.js`) — corrigé après le passage à healthsites.io (les anciens filtres "Dispensaires"/"24h" et l'ancienne taxonomie `hopital`/`clinique`/`maternite`/`dispensaire`/`public` ne matchaient plus les données réelles, voir Décisions techniques pour le détail des deux correctifs). `TYPE_ICON`/`TYPE_COLOR`/`TYPE_BG` couvrent `hopital`/`clinique`/`pharmacie`/`centre_sante` ; `autre` n'a pas d'entrée dédiée (retombe sur l'icône/couleur générique `map-pin`/`var(--sm-ink)`/`#F1F2F4`, catégorie fourre-tout jugée peu utile à distinguer visuellement). **Filtre "24h" retiré, pas remplacé** : `available24h` n'existe ni dans les données healthsites.io/OSM (aucun tag fiable et systématiquement présent) ni dans la table Supabase `health_centers` (colonne jamais ajoutée au schéma, y compris pour le seed manuel) — voir Décisions techniques pour la décision et ce qu'il faudrait pour le réintroduire honnêtement (nouvelle colonne nullable, jamais fabriquée). Appel direct par centre — bouton "Appeler" remplacé par "Numéro non disponible" (désactivé) si `phone: null`. Mention d'attribution discrète sous la carte (`map.osm_attribution`, licence ODbL — légalement requise, les tuiles ET les centres de santé sont des données OpenStreetMap ; le badge Leaflet natif reste désactivé, `attributionControl: false`, pour rester cohérent avec le style de l'app). Si la géolocalisation échoue en `PERMISSION_DENIED` **et** que `isIOSDevice()` détecte iOS (userAgent, avec le cas iPad qui se présente en `Macintosh` + `maxTouchPoints`), remplace le bouton "Réessayer" (inopérant sur iOS une fois le refus enregistré) par les instructions manuelles Réglages → Safari → Position — "Réessayer" reste actif pour Android et les autres cas. Le bandeau d'erreur GPS utilise `<Banner variant="warning" icon="alert-circle">` avec `children` (titre + instructions/bouton conditionnels selon `denied`/iOS). `FloatingChatButton`. |
...
| Centres de santé : nouvelle taxonomie `type`, `screen-map.jsx` d'abord laissé en retard puis corrigé | 5 catégories (`hopital`/`clinique`/`pharmacie`/`centre_sante`/`autre`) déduites des tags OSM, remplacent l'ancienne taxonomie (`hopital`/`clinique`/`dispensaire`/`public`/`maternite`). Un premier lot (8d6567a) avait volontairement laissé `screen-map.jsx` inchangé (hors périmètre à ce moment-là) ; un lot suivant a aligné `FILTERS`/`TYPE_ICON`/`TYPE_COLOR`/`TYPE_BG` sur la nouvelle taxonomie ("Dispensaires" → "Pharmacies" + "Centres de santé" ; `autre` volontairement sans chip ni icône dédiée, catégorie fourre-tout déjà couverte par "Tous") | Un filtre qui ne matche plus aucune donnée réelle silencieusement (comme "Dispensaires" l'était devenu) est pire qu'un filtre manquant — il laisse croire qu'une recherche a été faite alors qu'elle ne peut structurellement rien trouver. Vérifié avec les 20 entrées de seed migrées (2 hôpitaux, 11 cliniques, 7 centres de santé, 0 pharmacie) dans un navigateur réel (Playwright) : chaque filtre renvoie exactement le compte attendu, "Pharmacies" affiche proprement l'état vide existant plutôt que de planter |
| Centres de santé : filtre "24h" retiré, pas réparé | `available24h` retiré de `screen-map.jsx` (filtre ET badge) plutôt que réinterprété | Cette information n'existe de façon fiable ni dans les données healthsites.io/OSM (pas de tag systématiquement présent pour ça) ni dans la table Supabase `health_centers` elle-même — **aucune colonne de ce type n'a été ajoutée au schéma**, y compris pour le seed manuel (les 20 lignes de `supabase/schema.sql` n'ont pas de colonne `available24h` du tout). Réintroduire ce filtre honnêtement nécessiterait une vraie migration de schéma (nouvelle colonne nullable, jamais fabriquée pour les lignes où l'information est inconnue) — pas fait ici, volontairement, pour rester dans le périmètre d'un correctif de filtre plutôt que d'ouvrir une migration de schéma non demandée. Un filtre "24h" qui ne renverrait que 2 résultats sur toute la Côte d'Ivoire (les seuls hôpitaux du seed qui l'avaient) risquerait de laisser croire à tort qu'aucune autre option 24h n'existe à proximité — inacceptable dans le contexte d'une app de premiers secours |
