...
| `screen-home.jsx` | `HomeMobile` : logo `logo_80.png` en haut à gauche, carte IA bleue "Que se passe-t-il ?" (min-height 120px, classe `sm-card-breathe` — respiration douce du box-shadow en boucle pour attirer l'oeil sans agressivité, voir styles.css), QR + conseil (icônes noires sur fond #F1F2F4), `HomeTabBar` dégradé bleu #1565C0→#0D47A1 avec backdrop-blur + indicateur blanc sur onglet actif, onglet Localisation actif. Conseil du jour affiché via `<Banner variant="success" stacked>` (titre sur sa propre ligne, description en dessous — pas le mode inline utilisé pour les messages courts). Avatar cliquable → profil. Racine `<div position:absolute;inset:0>` (pas un Fragment — nécessaire pour que le safe-area CSS centralisé s'applique, voir Décisions techniques). `HomeDesktop` · `Sidebar`. |
...
| `screen-training-module.jsx` | `TrainingModuleScreen` — détail d'un module + quiz progressif (5 à 20 questions selon le module), soumission du score à `POST /training/:moduleId/complete`, déverrouille le module suivant si réussite ≥ 60%. Texte des étapes en 16px (relisibilité). `useEffect` sur `mod?.id` qui réinitialise `phase`/`result` à chaque changement de module — sinon `nav.go('training_module')` vers le module suivant réutilisait le composant déjà monté et restait bloqué sur l'écran de résultat de l'ancien module. `QuizPhase` garde en mémoire (`wrongAnswersRef`) chaque question ratée (texte, réponse donnée, bonne réponse), transmis à `ResultPhase` via le résultat. Sur l'écran de résultat, dès que le score n'est pas 100% : section "Questions à revoir" listant chaque erreur (réponse donnée en rouge ✗, bonne réponse en vert ✓). Boutons : échec (< 60%) → "Recommencer le quiz" (`handleRetry`, repasse par les étapes) ; validé mais imparfait (60–99%) → "Module suivant" reste l'action principale (si dispo), + bouton secondaire "Revoir mes erreurs et refaire le quiz" (`handleRetryQuiz`, saute directement au quiz sans repasser par les étapes — la matière est déjà maîtrisée). |
...
| QR médical : expiration en query string | `exp`/`gen` (timestamps ms) portés par l'URL elle-même plutôt que stockés côté serveur | La route publique n'a pas de session pour retrouver la date de génération d'un QR donné ; évite une migration de schéma Supabase pour un simple horodatage cosmétique |
